<template>
  <div class="row">
    <div class="col-xl-4">
      <div class="card mb-4 mb-xl-0" style="margin-left: 150px;margin-top:20px">
        ...
        <div class="card-body text-center">
          ...
          <label for="formFile" class="form-label">Mettre à jour votre photo de profil</label>

          <input class="form-control" type="text" id="imgUrl" placeholder="URL d'image" v-model="user.image"/>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    if (this.user.dateNaissance != null) {
      ...
      this.user.dateNaissance = date.toISOString("en-US", {month: "2-digit", day: "2-digit"}).split('T')[0];
    } else {
  ...
  methods: {
    testCurrentPassword(ev) {
      ...
      fetch(`http://localhost:8080/api/user/testCredentials/${this.user.email}/${this.user.password}`, {
        method: "GET",
